...
PreprocessHint = Literal["none", "standard", "minmax"]
...
_REGISTRY: dict[str, EstimatorSpec] = {
    ...
    "svc": EstimatorSpec(
        "svc", "Support Vector Classifier", _svc,
        [
    ...
    "nsvc": EstimatorSpec("nsvc", "Nu-SVC", _nusvc, [{"nu": [0.25, 0.5, 0.75], "kernel": ["rbf", "linear"]}], preprocess="standard"),
    "gp": EstimatorSpec(
        "gp", "Gaussian Process Classifier", _gp,
        [{"kernel": [RBF(0.5), RBF(1.0), Matern(length_scale=1.0, nu=1.5), RationalQuadratic(), DotProduct()]}],
        preprocess="standard",
    ),
    "dt": EstimatorSpec("dt", "Decision Tree", _dt, [{"criterion": ["gini", "entropy", "log_loss"], "max_depth": [None, 3, 5, 10]}]),
    "rf": EstimatorSpec(
        "rf", "Random Forest", _rf,
    ...
    "hgbc": EstimatorSpec(
        "hgbc", "Histogram Gradient Boosting", _hgbc,
        [{"max_iter": [100, 125, 135], "learning_rate": [0.1, 0.2, 0.3], "l2_regularization": [0.0, 1.0, 2.0]}],
    ),
    "ada": EstimatorSpec("ada", "AdaBoost", _ada, [{"n_estimators": [50, 100, 125, 135], "learning_rate": [0.1, 0.2, 0.3, 1.0]}]),
    "qda": EstimatorSpec("qda", "Quadratic Discriminant Analysis", _const(QuadraticDiscriminantAnalysis), [{"reg_param": [0.0, 0.5, 1.0]}], preprocess="standard"),
    "lda": EstimatorSpec(
        "lda", "Linear Discriminant Analysis", _const(LinearDiscriminantAnalysis),
        [
            {"solver": ["svd"]},
    ...
    "etc": EstimatorSpec(
        "etc", "Extra Trees", _etc,
        [{"n_estimators": [50, 100, 150], "criterion": ["gini", "entropy", "log_loss"], "max_features": ["sqrt", "log2", None], "min_samples_split": [2, 3, 5, 6], "min_samples_leaf": [1, 2, 3, 4, 5]}],
    ),
    "gnb": EstimatorSpec("gnb", "Gaussian Naive Bayes", _const(GaussianNB), [{"var_smoothing": [1e-7, 1e-8, 1e-9, 1e-10]}]),
    "mnb": EstimatorSpec("mnb", "Multinomial Naive Bayes", _const(MultinomialNB), [{"alpha": [0.0, 0.8, 0.9, 1.0, 1.1], "force_alpha": [True, False]}], preprocess="minmax"),
    "compnb": EstimatorSpec("compnb", "Complement Naive Bayes", _const(ComplementNB), [{"alpha": [0.0, 0.8, 0.9, 1.0, 1.1], "force_alpha": [True, False]}], preprocess="minmax"),
    "bnb": EstimatorSpec("bnb", "Bernoulli Naive Bayes", _const(BernoulliNB), [{"alpha": [0.0, 0.8, 0.9, 1.0, 1.1], "force_alpha": [True, False]}], preprocess="minmax"),
    "lsvc": EstimatorSpec(
        "lsvc", "Linear SVC", _lsvc,
        [
