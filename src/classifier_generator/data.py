...
def load_csv_dataset(paths: Sequence[str | Path], label_column: str = "label") -> tuple[pd.DataFrame, pd.Series]:
    """Load and concatenate one or more CSV files with a shared label column."""
    if not paths:
...
def split_dataset(
    ...
    *,
    ...
    """Create one stratified holdout split. The holdout is not used during model selection."""
